...
const Home = ({status, expand}) => {
  return (
    <div className='container'>
      <div >
        <section >
          <h2 id='home-header' className={status ? 'header-toggle expanded' : 'header-toggle'} onClick={() => expand('home')}>
            <span className='header-span'  >Home Value</span>
            {status ? <img className="chev" src='./down.png'/>
              : <img className="chev" src='./up.png'/>}
          </h2>
          {status && (<div className='sub-container' id='home-active'>
            <div >
              <ul >
                <li >
